...
async function evaluateNFTRisk(walletAddress, contractAddress, chainId) {
    const headers = {
        'X-Sim-Api-Key': YOUR_API_KEY,
        'Content-Type': 'application/json'
    };
    ...
    try {
        ...
        const [nfts, activity, holders, balances, tokenInfo] = await Promise.all([
            // 1. Get NFT ownership data
            fetch(`https://api.sim.dune.com/v1/evm/collectibles/${walletAddress}`, { headers })
                .then(r => r.json()),
            
            // 2. Get transaction activity
            fetch(`https://api.sim.dune.com/v1/evm/activity/${walletAddress}?limit=50`, { headers })
                .then(r => r.json()),
            
            // 3. Get holder distribution
            fetch(`https://api.sim.dune.com/v1/evm/token-holders/${chainId}/${contractAddress}`, { headers })
                .then(r => r.json()),
            
            // 4. Get wallet portfolio
            fetch(`https://api.sim.dune.com/v1/evm/balances/${walletAddress}`, { headers })
                .then(r => r.json()),
            
            // 5. Get token metadata
            fetch(`https://api.sim.dune.com/v1/evm/token-info/${contractAddress}?chain_ids=${chainId}`, { headers })
                .then(r => r.json())
        ]);

        // Calculate risk metrics
        const riskMetrics = {
            collectionHealth: calculateCollectionHealth(holders),
            walletRisk: calculateWalletRisk(balances, activity),
            liquidityRisk: calculateLiquidityRisk(tokenInfo),
            activityRisk: calculateActivityRisk(activity)
        };

        return riskMetrics;
    } catch (error) {
        console.error('Error evaluating NFT risk:', error);
        throw error;
    }
}

console.log("risk matrics:",evaluateNFTRisk("0xb47e3cd837ddf8e4c57f05d70ab865de6e193bbb", "0xb47e3cd837ddf8e4c57f05d70ab865de6e193bbb", 1) )