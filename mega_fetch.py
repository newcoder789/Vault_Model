...
# 2. Fetch Collection Slugs
def fetch_collection_slugs(limit=100, offset=0, max_retries=3):
    url = f"{BASE_URL_OPENSEA}/collections"
    ...
    for attempt in range(max_retries):
        try:
            ...
            print(f"Network error: {e}. Attempt {attempt + 1} of {max_retries}")
            time.sleep(2**attempt)
    return []


# Fetch all collection slugs (paginated)
all_slugs = [
    "boredapeyachtclub",
...
# offset = 0
# while True:
#     slugs = fetch_collection_slugs(offset=offset)
#     if not slugs:
#         break
...
if not all_slugs:
    print("No collection slugs found. Check API key or endpoint.")
    sys.exit(1)

print(f"Found {len(all_slugs)} collection slugs.")


# 3. Fetch NFTs from Collections (OpenSea)
def fetch_collection_nfts(slug, limit=100, offset=0, max_retries=3):
...
all_nfts_opensea = []
for slug in all_slugs[:5]:  # Limit to 5 for now to avoid rate limits
    offset = 0
    while True:
